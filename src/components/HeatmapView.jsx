    ...
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      <div style={card}>
        ...
        <p style={{ margin:"0 0 14px", fontSize:12, color:"var(--t-label)", fontFamily:"sans-serif" }}>
          Each cell = one day. Darker = more habits completed.
        </p>
        <div style={{ display:"flex", gap:4, marginBottom:6 }}>
          {DAY_NAMES.map(d => (
            <div key={d} style={{ flex:1, textAlign:"center", fontSize:9, color:"var(--t-muted)", fontFamily:"sans-serif" }}>{d}</div>
          ))}
        </div>
              ...
              {Array.from({ length: 7 }, (_, ci) => {
                ...
                return (
                  <div key={ci} title={`Day ${cell.idx + 1}: ${pct}%`} style={{
                    ...
                    background: `rgba(${theme.accentRgb},${alpha})`,
                    border: isToday ? `2px solid var(--t-accent)` : "2px solid transparent",
                    display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center",
                    opacity: future ? 0.3 : 1,
                  }}>
                    <div style={{ fontSize:11, fontFamily:"sans-serif", color:pct>50?"#fff":"var(--t-strong)", fontWeight:700, lineHeight:1 }}>{cell.idx + 1}</div>
                    {!future && <div style={{ fontSize:8, fontFamily:"sans-serif", color:pct>50?"rgba(255,255,255,0.8)":"var(--t-muted)", marginTop:1 }}>{pct}%</div>}
                  </div>
                );
              })}
            </div>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:5, marginTop:14 }}>
          <span style={{ fontSize:10, color:"var(--t-muted)", fontFamily:"sans-serif" }}>Less</span>
          {[0, 25, 50, 75, 100].map(p => (
            <div key={p} style={{ width:18, height:18, borderRadius:4, background:`rgba(${theme.accentRgb},${0.08 + p / 100 * 0.9})` }} />
          ))}
          <span style={{ fontSize:10, color:"var(--t-muted)", fontFamily:"sans-serif" }}>More</span>
        </div>
      </div>
              ...
              <div style={{ display:"flex", gap:2 }}>
                {Array.from({ length:30 }, (_, d) => (
                  <div key={d} style={{
                    ...
                    background: (checked[h.id] || [])[d] ? cat.color : "var(--t-light)",
